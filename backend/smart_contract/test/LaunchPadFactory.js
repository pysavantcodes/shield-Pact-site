...
describe("Launch Factory", ()=>{
	...
	before(async ()=>{
		...
		await Token1.mint(signers[1].address,ethers.utils.parseUnits("1000000",18));
		...
		Token2.deployed();
		await Token2.mint(signers[2].address,ethers.utils.parseUnits("1000000",18));
	...
	it("create Launchpad", async()=>{
		...
		const capped = parseEther("500");
	
		const pu = (v)=>parseUnits(v, 18);
		let data = await contract.totalTokenNeeded(capped, pu("200"), pu("150"), 80);
		//console.log(data);
		console.log("=> ",ethers.utils.formatUnits(data[0],18));	
		let result = await Token1.approve(contract.address, data[0]);
		await result.wait();

		result = await contract.createPad(Token1.address, true, pu("200"), pu("150"), 80, capped, [minBuy, maxBuy], 
